...
export class RolesAdminView {
    ...
    constructor() {
        ...
        if (!table) throw new Error("Không tìm thấy #table_role");
        this.tableBody = table as HTMLElement;
    }

    render(roles: UserRole[], users: User[]) {
    ...
    // ADD
    onCreate(callback: (name: string) => void) {
        const btn = document.querySelector(".add-role-btn");
        const input = document.querySelector(".role-name-input") as HTMLInputElement;
    ...
    // SAVE EDIT
    onSave(callback: (id: string, name: string) => void) {
        this.tableBody.addEventListener("click", (e) => {
            const target = e.target as HTMLElement;

            if (target.classList.contains("save-btn")) {
                const id = target.dataset.id!;
                const input = this.tableBody.querySelector(`.edit-role-input[data-id="${id}"]`) as HTMLInputElement;
