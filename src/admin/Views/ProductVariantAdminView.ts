...
export class ProductVariantAdminView {
    private tableBody = document.querySelector("#variant-table-body") as HTMLElement;

    render(variants: ProductVariantAdmin[], productMap: Map<string, string>) {
        this.tableBody.innerHTML = variants
            .map(
                ...
                <tr class="border-b hover:bg-gray-50 ${v.status === "inactive" ? "opacity-50" : ""}">
                    ...
                    <td>${productMap.get(v.product_id) ?? "Unknown"}</td>
